...
Date: 2024-02-01 22:10:13
LastEditors: EPAICAR EPAICAR@EPAICAR
LastEditTime: 2024-02-03 23:24:56
FilePath: /tls_visualtask/Unet-3.4/src/infer.py
...
def get_angle(img):
    ...
    # cv2.imwrite("../run/infer/img.jpg",cropped)
    # print(angle)
...
if __name__ == "__main__":
    ...
    weights = '../pre_model/UNet_Fire.pth'                   #权重路径+名称
    _input='../demo'    #测试集路径
    # num_classes = 6 #标签数量
    ...
    size = (320, 240)
    pipeline.set(cv2.CAP_PROP_FRAME_HEIGHT, size[0])
    pipeline.set(cv2.CAP_PROP_FRAME_WIDTH, size[1])
    ...
    if os.path.exists(weights):
        ...
        while True:
            ...
            img_data=transform(img_pil).cuda()
            ...
            out=out*255.0
            # save_path_img = os.path.join(save_path,'img.png')
            # save_path_out = os.path.join(save_path,'infer.png')
            # cv2.imwrite(save_path_out,out)
            # cv2.imwrite(save_path_img,frame)
